...
export const getJobPosts = async (req, res, next) => {
  try {
    ...
    if (search) {
      const searchQuery = {
        $or: [
          { jobTitle: { $regex: search, $options: "i" } },//i means case insensitive
          { jobType: { $regex: search, $options: "i" } },//i means case insensitive
        ],
      };
    ...
    let queryResult = Jobs.find(queryObject).populate({
      path: "company",// because we want to get company information
      select: "-password",//because we don't want to send password to client
    });

...
const CompnayForm = ({ open, setOpen }) => {
  ...
  const {
    ...
    mode: 'onChange',
    defaultValues: { ...user}, // iski waja sa wo edit page me data a rah tha ...user.?user. asa tha koch
  })
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res.status === 'failed') {
        ...
        //  yahan changing ki hein .. agr koi masla ata ha company profile me to yahan per a kr dekh skty hein
      
        // localStorage.setItem('userInfo', JSON.stringify(newData))// iska mene change kia ha usny ni kiaa lakin meny commnt ker dia ha 
        // localStorage.setItem('userInfo', JSON.stringify(data))// iskay sath window likha ha meny nichy..
        setTimeout(() => {
          window.location.reload()
        }, 1500)
      }
...
const CompanyProfile = () => {
  ...
  const fetchCompany = async () => {
    ...
    {
      // id = user?.user?.id// pencil iski waja sa error a rha tha
      id = user?._id
    }
    try {
      const res = await apiRequest({
        // url:"/company/get-company/"+id,
        url: "/company/get-company/"+ id, // ni chaidl rha meny id laga ker dekhi ha
        method: 'GET',
      })
      setInfo(res?.data)
      setIsLoading(false)
      ...
      <div className="">
        <div className="w-full flex flex-col md:flex-row gap-3 justify-between">
          ...
          {user?.user?.accountType === undefined &&
            // info?._id === user?.user?._id && (// isko bhi change kia to sahi tarah chal para ha 
            info?._id === user?._id && (
      ...
      <div className="w-full mt-20 flex flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {info?.jobPosts?.map((job, index) => {
            {
              /*  and this one is for correct data */
            }
            {
              /* iska lagany sa chala para ha matlab this is for damy data */
            }

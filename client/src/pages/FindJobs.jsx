 ...
 const fetchJobs = async ()=>{
  ...
  useEffect(()=>{
    ...
    if(expVal.length > 0){
      let newExpVal=[];
      expVal?.map((el)=>{  
        const newEI= el?.split("-");
        newExpVal.push(Number(newEI[0]),Number(newEI[1]));
      });
          ...
          {numPage > page && !isFetching && (
            <div className='w-full flex items-center justify-center pt-16'>
              <CustomButton
                ...
                title='Load More'
                containerStyles={`text-blue-600 py-1.5 px-5 focus:outline-border hover:bg-blue-700 hover:text-white rounded-full text-base border border-blue-600`}
